...
public:
    ...
    *           < 0     failed
    */
    int16_t tgInitAsTarget(uint16_t timeout = 0);
    int16_t tgInitAsTarget(const uint8_t* command, const uint8_t len, uint8_t *mode, uint8_t *initiatorcommand, const uint16_t timeout = 0);
    ...
    int16_t tgResponseToInitiator(const uint8_t *data_out, uint8_t len, const uint16_t timeout);
    int16_t tgGetInitiatorCommand(uint8_t *data_in, uint8_t len, const uint16_t timeout);
